...
// (9. Reprovado na Entrevista), então os chips recortam por sinalização/atraso.
...
// aplica os filtros sobre allRejectedInterviewForms. `ignoreTriage` é usado para contar os chips:
...
function getFilteredForms(filters, { ignoreTriage = false } = {}) {
    return allRejectedInterviewForms.filter(form => {
...
// ordena uma cópia da lista para não mexer na ordem original de allRejectedInterviewForms
...
function updateSortIndicators() {
    document.querySelectorAll(".rejected-interview-table th.is-sortable").forEach(th => {
...
function updateRejectedInterviewCount(shown, total) {
    const counter = document.getElementById("rejectedInterviewCount");
...
function renderRejectedInterviewFormsTable(forms, filters) {
    const tbodyRejected = document.getElementById("tbodyRejectedInterviewForms");
...
function applyFilters() {
    ...
    renderRejectedInterviewFormsTable(sortForms(filtered), filters);
    updateSortIndicators();
    updateRejectedInterviewCount(filtered.length, allRejectedInterviewForms.length);
...
// função para preencher tabela de formulários reprovados na entrevista
export async function populateRejectedInterviewFormsTable() {
    const tbodyRejected = document.getElementById("tbodyRejectedInterviewForms");

    try {
        const response = await fetchWithAuth(`/entrevista-adesao/application-forms/status?status_id=9`);
        ...
        allRejectedInterviewForms = await response.json();

        populateFilterOptions(allRejectedInterviewForms);
        applyFilters();
    } catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const thead = document.querySelector(".rejected-interview-table thead");
