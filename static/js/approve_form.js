...
// atalho de triagem selecionado. Diferente da /fichas, aqui todas as fichas têm o mesmo status
// (1. Aguardando aprovação financeira), então os chips recortam por sinalização/atraso, não por status.
...
function updateSortIndicators() {
    document.querySelectorAll(".approve-table th.is-sortable").forEach(th => {
...
function updateApprovalCount(shown, total) {
...
function renderApprovalTable(forms, filters) {
    ...
    if (forms.length === 0) {
        ...
        tbodyPending.innerHTML = hasActiveFilters(filters)
            ...
            : `<tr class="forms-empty-row"><td colspan="7">Nenhuma ficha aguardando aprovação financeira.</td></tr>`;
        return;
    }
...
function applyFilters() {
    ...
    renderApprovalTable(sortForms(filtered), filters);
    updateSortIndicators();
    updateApprovalCount(filtered.length, allPendingForms.length);
...
export async function populateFormsApproveTable() {
    ...
    try {
        ...
        tbodyPending.innerHTML = `<tr class="forms-empty-row"><td colspan="7">Não foi possível carregar os formulários.</td></tr>`;
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const thead = document.querySelector(".approve-table thead");
